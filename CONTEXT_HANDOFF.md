...
- CI 已改为：tag 构建后自动把 APK 发布到 GitHub Release，避免手动下载 artifact 再上传。
...
- v1.0.9 已通过 tag 触发 CI 自动发布 Release APK，下载链接已验证返回 `200`。

# 未完成事项
- 真机连接 ADB 后再做完整二维码流程测试。

# 下一步
- 用户下载并安装 `v1.0.9`：`https://github.com/pythonzjw/visitor_qr_register/releases/download/v1.0.9/visitor-qr-register-v1.0.9.apk`。
- 真机 ADB 可用后继续完整流程测试。
...
# 本次发布
- 2026-06-13：提交 `f594b14`，tag `v1.0.9` 已推送；Release APK 已由 CI 自动发布，链接可访问。
